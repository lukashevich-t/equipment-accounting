...
import by.gto.equipment.account.helpers.guidAsBytes
import by.gto.equipment.account.helpers.guidFromBytes
...
//    }
    ...
    private fun equipmentDescrFromResultSet(rs: ResultSet): EquipmentDescr {
        ...
        r.guid = rs.getBytes("id").guidFromBytes().toString()
...
serial = VALUES(equipment.serial)"""
    ...
    open fun getEquipmentsByGuidList(sGuids: List<String?>?): List<Equipment> {
        ...
        for (sGuid in sGuids) {
            try {
                bGuids.add(UUID.fromString(sGuid).guidAsBytes())
                cond.append(" e.id = ? or")
            } catch (ignored: Exception) {
            }
    ...
    open fun logEquipmentChange(
        ...
        return QueryRunner(ds)
            .update(
                ...
                guid.guidAsBytes(), action.id, userId, comment, d
            )
    }

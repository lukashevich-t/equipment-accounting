<template>
  <q-page>
    ...
    <q-dialog v-model="AssociationShowing">
      <Association-dlg
        @Association="Association"
        :criteria="criteria"
        :AssociationCrit="AssociationCrit"
      />
    </q-dialog>
    ...
    <span class="fixed-top-right q-mb-sm" style="z-index: 9999;">
      ...
      <q-btn
        ...
        @shortkey="openAssociationDialog"
      />
      <q-btn
        dense
        icon="print"
...
<script>
import AssociationDlg from 'src/modals/AssociationDlg'
...
export default {
  ...
  components: { AssociationDlg, EditDlg, AddMaintenanceDlg, LogDlg },
  computed: {
    ...mapGetters(['selectedGuids']),
  },
  methods: {
    help() {
      alert(
    ...
    Association(c, d) {
      ...
      let vm = this
      vm.AssociationShowing = false
      vm.loading = true
      vm.criteria = c
      vm.result = []
      vm.selected = []
      vm.AssociationCrit = d
      ajax
        .AssociationEq(vm.criteria, vm.AssociationCriteria)
        .then(
          r => {
